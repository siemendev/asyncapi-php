...
class Generator
{
    ...
    private function clearStubDirectory(string $outputPath): void
    {
        ...
        foreach ($files as $file) {
            if (is_file($file)
                && str_contains(file_get_contents($file, length: 150) ?: '', '@asyncapi-php-generated')
            ) {
                unlink($file);
            }
        }
    ...
    private function generateClass(
        ...
        $fileName = $configuration->getPath() . DIRECTORY_SEPARATOR . $configuration->getClassName($message) . '.php';

        $file = new PhpFile();
        $file->addComment('This file is @generated by AsyncAPI PHP Generator. Do not edit it manually. @asyncapi-php-generated');

        $class = $file->addClass($configuration->getMessageClass($message))
            ->setFinal(false)
        ...
        $requiredProperties = [];
        $optionalProperties = [];
        foreach ($message->resolvePayload()?->resolveProperties() ?? [] as $name => $property) {
            ...
            if (in_array($name, $message->resolvePayload()?->getRequired() ?? [], true)) {
